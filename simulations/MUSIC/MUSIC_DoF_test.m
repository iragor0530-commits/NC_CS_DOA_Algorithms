%% --- 基于非圆扩展 MUSIC 的自由度压力测试 (M = 3, 5, 8, 10, 12, 15) ---
clear; clc;
...
for i = 1:length(M_test_list)
    ...
    theta_curr = sort(generate_angles(M_curr, -60, 60, 4.5));
    ...
    % 4. 调用 NC_MUSIC 算法
    fprintf('正在计算 M = %2d 的空间谱 (MUSIC)...\n', M_curr);
    [P_db_curr, ~] = NC_MUSIC(Y_curr, grid_theta, M_curr);
...
sgtitle(['基于非圆信号扩展的 NC-MUSIC 自由度测试 (物理阵元 N=8, SNR=', ...
